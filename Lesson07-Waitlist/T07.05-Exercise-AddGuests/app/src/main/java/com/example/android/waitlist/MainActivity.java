...
public class MainActivity extends AppCompatActivity {
    ...
    public void addToWaitlist(View view) {
        Editable partySizeEdit = mNewPartySizeEditText.getText();
        Editable partyNameEdit = mNewGuestNameEditText.getText();
        // complete (9) First thing, check if any of the EditTexts are empty, return if so
        if(partySizeEdit.length()==0 || partyNameEdit.length()==0) return;
        ...
        try{
            partySize = Integer.parseInt(partySizeEdit.toString());
        }
        catch(Exception ex){
            Log.i(LOG_TAG,"Failed to parse partySize in addToWaitlist. exception: "+ex.toString());
        }
        ...
        addNewGuest(partyNameEdit.toString(),partySize);
